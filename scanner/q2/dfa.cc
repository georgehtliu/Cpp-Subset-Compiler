...
struct State {
  ...
  std::pair<std::string, bool> getNextState(char input) {
    ...
    if (neighbors.empty() || neighbors.count(input) == 0) return std::make_pair("", false);
...
bool accepts(std::string inputString, std::map<std::string, State> states, std::string initialState) {

  State curr = states.at(initialState);

  for (int i = 0; i < inputString.size(); i++) {

    char c = inputString.at(i);
    if (!curr.getNextState(c).second) return false;
    std::string nextStateName = curr.getNextState(c).first;
    State nextState = states.at(nextStateName);
    curr = nextState;
...
int main() {
  ...
  while(in >> s) {
    ...
    if (accepts(s, states, initialState)) {
